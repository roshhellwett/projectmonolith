...
def get_group_settings_keyboard(chat_id: int, group_settings: dict = None) -> InlineKeyboardMarkup:
    ...
    rows = [
        [
            InlineKeyboardButton(f"🤖 Anti-Spam {'✅' if anti_spam else '❌'}", callback_data=f"grp_toggle_spam_{chat_id}"),
        ],
        [
            InlineKeyboardButton(f"🔞 Anti-Abuse {'✅' if anti_abuse else '❌'}", callback_data=f"grp_toggle_abuse_{chat_id}"),
        ],
        [
            InlineKeyboardButton(f"🌊 Flood Control {'✅' if flood_control else '❌'}", callback_data=f"grp_toggle_flood_{chat_id}"),
        ],
