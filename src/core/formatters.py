...
def format_card(title: str, body: str, footer: str | None = None, border_color: str = "🟦") -> str:
    """
    ...
    if footer:
        lines.extend(["━" * 20, f"<i>{footer}</i>"])

    return "\n".join(lines)


def format_list_item(number: int, text: str, icon: str = "•", max_length: int = 60) -> str:
...
def format_usage_meter(current: int, limit: int, label: str = "Usage", show_numbers: bool = True) -> str:
...
def format_price_change(change: float, percentage: float | None = None, show_arrow: bool = True) -> str:
    """
...
def format_pnl(pnl: float, percentage: float | None = None, show_color: bool = True) -> str:
    """
    ...
    """
    emoji = ("\U0001f7e2" if pnl >= 0 else "🔴") if show_color else ""
...
def format_telegram_user(user_id: int, first_name: str | None = None, username: str | None = None) -> str:
...
def format_key_value(key: str, value: str, key_width: int = 15, justify: bool = True) -> str:
...
def format_bullet_list(items: list[str], emoji: str = "•", max_items: int | None = None) -> str:
...
def format_numbered_list(items: list[str], start: int = 1, max_items: int | None = None) -> str:
...
def format_analytics_summary(total_actions: int, top_category: str, top_user: str, period: str = "24h") -> str:
...
def format_feature_locked(feature: str, upgrade_cta: str = "Upgrade to PRO") -> str:
...
def format_limit_reached(feature: str, current: int, limit: int, upgrade_benefit: str = "unlimited") -> str:
...
def format_already_done(action: str, details: str = "") -> str:
...
def format_success(action: str, details: str | None = None, emoji: str = "✅") -> str:
...
def format_warning(message: str, suggestion: str | None = None) -> str:
