...
async def update_loading_dots(
    message, context: ContextTypes.DEFAULT_TYPE, base_text: str, dot_count: int = 3, max_iterations: int = 6
) -> None:
    """Update a message with animated dots to show progress."""
    for i in range(max_iterations):
        dots = "." * ((i % dot_count) + 1)
        try:
            await message.edit_text(text=f"{base_text}{dots}", parse_mode="HTML")
        except BadRequest:
            break
...
async def edit_with_stages(
    ...
    for stage in stages:
        try:
            if msg is None:
                if update.callback_query:
                    msg = await update.callback_query.edit_message_text(text=f"⏳ {stage}...", parse_mode="HTML")
                elif update.message:
                    msg = await update.message.reply_text(text=f"⏳ {stage}...", parse_mode="HTML")
            else:
                msg = await msg.edit_text(text=f"⏳ {stage}...", parse_mode="HTML")
        except BadRequest:
            pass
        await asyncio.sleep(delay)

    return await edit_with_animation(update, context, final_text, final_keyboard)


async def show_progress_bar(
    update: Update, context: ContextTypes.DEFAULT_TYPE, title: str, current: int, total: int, unit: str = "items"
) -> Any | None:
...
def create_processing_keyboard(
    cancel_callback: str = "proc_cancel", cancel_text: str = "✖ Cancel"
) -> InlineKeyboardMarkup:
    """Create a keyboard showing processing state with cancel option."""
    keyboard = [[InlineKeyboardButton("⏳ Processing...", callback_data="proc_none")]]
    if cancel_callback:
...
def create_retry_keyboard(
    retry_callback: str = "retry_action", help_callback: str = "get_help"
) -> InlineKeyboardMarkup:
...
def create_undo_keyboard(
    undo_callback: str, undo_text: str = "↩️ Undo", timeout_seconds: int = 10
) -> InlineKeyboardMarkup:
    """Create an undo keyboard with timeout indication."""
    keyboard = [[InlineKeyboardButton(f"{undo_text} ({timeout_seconds}s)", callback_data=undo_callback)]]
    return InlineKeyboardMarkup(keyboard)
...
async def delete_message_safe(message, context: ContextTypes.DEFAULT_TYPE) -> bool:
