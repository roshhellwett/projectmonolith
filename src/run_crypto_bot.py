...
async def cmd_activate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        return await update.message.reply_text(
            "⚠️ <b>Invalid Format.</b> Use: <code>/activate [YOUR_KEY]</code>", parse_mode="HTML"
        )
...
async def handle_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if query.data == "ui_main_menu":
            ...
            status = (
                f"🟢 <b>Active</b> — {days_left} days remaining" if is_pro else "🔴 <b>Inactive</b> — Standard Tier"
            )
            pro_features = (
                "\n<b>Pro Features:</b>\n"
            ...
            if not audits:
                await query.edit_message_text(
                    "🗂️ <b>Audit Vault</b>\n\nEmpty. Run a scan: <code>/audit [contract]</code>",
                    reply_markup=get_back_button(),
                    parse_mode="HTML",
                )
            else:
                await query.edit_message_text(
                    "🗂️ <b>Audit Vault</b>\n\nSelect a record:",
                    reply_markup=get_audits_keyboard(audits),
                    parse_mode="HTML",
                )
            ...
            if not audits:
                await query.edit_message_text(
                    "🗂️ <b>Vault cleared.</b>", reply_markup=get_back_button(), parse_mode="HTML"
                )
            else:
                await query.edit_message_text(
                    "🗂️ <b>Record removed.</b>", reply_markup=get_audits_keyboard(audits), parse_mode="HTML"
                )

        elif query.data == "ui_clear_audits":
            await SubscriptionRepo.clear_all_audits(user_id)
            await query.edit_message_text(
                "🗂️ <b>Vault wiped.</b> All data erased.", reply_markup=get_back_button(), parse_mode="HTML"
            )
            ...
            lines = [
                ...
                f"<b>Fear & Greed:</b> {fng_val}/100 — <b>{fng_class}</b>",
                gauge,
                "",
            ...
            if not tokens:
                ...
                for t in tokens:
                    ...
                    ic = "🟢" if pnl >= 0 else "🔴"
                    lines.append(
                        f"{ic} <b>{t.token_symbol}</b> ×{t.quantity}\n   ${t.entry_price:,.2f}→${cp:,.2f} ({pnl:+.1f}%)\n"
                    )
            ...
            if not alerts:
                ...
                await query.edit_message_text(
                    "🔔 <b>Your Alerts</b>", reply_markup=get_alerts_keyboard(alerts), parse_mode="HTML"
                )
            ...
            if not alerts:
                await query.edit_message_text(
                    "🔔 Alert removed. No remaining alerts.", reply_markup=get_back_button(), parse_mode="HTML"
                )
            else:
                await query.edit_message_text(
                    "🔔 Alert removed.", reply_markup=get_alerts_keyboard(alerts), parse_mode="HTML"
                )
            ...
            if not is_pro:
                ...
                if not wallets:
                    ...
                    await query.edit_message_text(
                        "👁️ <b>Tracked Wallets</b>", reply_markup=get_wallets_keyboard(wallets), parse_mode="HTML"
                    )
            ...
            if not remaining:
                await query.edit_message_text(
                    "👁️ Wallet removed. No remaining trackers.", reply_markup=get_back_button(), parse_mode="HTML"
                )
            else:
                await query.edit_message_text(
                    "👁️ Wallet removed.", reply_markup=get_wallets_keyboard(remaining), parse_mode="HTML"
                )
...
async def active_blockchain_watcher():
    ...
    while True:
        ...
        explorer.get(network, "https://etherscan.io/tx/")
