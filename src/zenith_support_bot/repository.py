    ...
    default_faqs = [
        {
            ...
            "answer": "Use /activate [KEY] with your activation key to upgrade to Pro. Pro users enjoy unlimited tickets, AI auto-responses, priority support, and more.",
            "category": "billing",
        },
        {
            "question": "How do I create a ticket?",
            "answer": "Use /ticket [subject] | [description]. For example: /ticket Login Issue | I can't log into my account.",
            "category": "tickets",
        },
        {
            "question": "How do I check my ticket status?",
            "answer": "Use /status [TICKET_ID] to check the status of your ticket. You can also use /mytickets to see all your open tickets.",
            "category": "tickets",
        },
        {
            "question": "How do I close a ticket?",
            "answer": "Use /close [TICKET_ID] to close your own ticket. Only open tickets can be closed.",
            "category": "tickets",
        },
        {
            "question": "What are the ticket priorities?",
            "answer": "Pro users can set priorities: Low, Normal, High, Urgent. Higher priority tickets are addressed faster by our team.",
            "category": "tickets",
        },
    ]
...
class TicketRepo:
    ...
    async def set_in_progress(ticket_id: int) -> bool:
        async with AsyncSessionLocal() as session:
            stmt = (
                update(SupportTicket)
                .where(SupportTicket.id == ticket_id, SupportTicket.status.in_(["open", "in_progress"]))
                .values(
                    status="in_progress",
                    updated_at=utc_now(),
                )
...
class FAQRepo:
    ...
    async def search_faqs(query: str) -> list:
        async with AsyncSessionLocal() as session:
            stmt = (
                select(FAQEntry)
                .where((FAQEntry.question.ilike(f"%{query}%")) | (FAQEntry.answer.ilike(f"%{query}%")))
                .limit(10)
            )
            return (await session.execute(stmt)).scalars().all()
