...
async def check_single_bot(bot):
    ...
    last_alert = last_alert_time.get(alert_key)

    if (
        status == "error"
        and bot.status != "inactive"
        and (last_alert is None or (now - last_alert).total_seconds() > ALERT_COOLDOWN)
    ):
        await queue_alert(
