...
async def cmd_persona(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        personas_list = "\n".join(
            f"  {'✅' if k == current else '•'} <code>{k}</code> — {v['icon']} {v['name']}" for k, v in PERSONAS.items()
        )
...
async def cmd_research(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await edit_with_stages(
            update, context, stages=stages, final_text="🔬 <i>Research complete! Compiling report...</i>", delay=0.8
        )
